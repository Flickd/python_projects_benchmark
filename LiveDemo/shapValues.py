...
from config import BASE_PATH, SET_PATH, freq_bands, methods
...
for ts in training_sets:
    for sv in set_vary:
        ...
        explainer = shap.KernelExplainer(
            best_model.predict, shap.sample(x_train_df, 5), num_jobs=-1
        )

        # Compute Shap values for all instances in X_test
        shap_values = explainer.shap_values(x_test_df)

        shap_dict = dict(shap_values=shap_values, fold=skf_vals[best_fold])
